...
if not pygame.font: raise SystemExit('Não conseguiu carregar fontes')
...
class Mundo():
    ...
    tam_x = property(get_tamx)
    tam_y = property(get_tamy)
    ...
    def get_vazio(self, x, y, objeto=None): # retorna se a posição está vazia
...
class Objeto(pygame.sprite.Sprite):
    ...
    def redimensiona(self, tamanho):
        ...
        if width > tamanho or height > tamanho:
            if width > height:
                height = int(round((tamanho*1.0)/width * height, 0))
                width = tamanho
            else:
                width = int(round((tamanho*1.0)/width * height, 0))
                height = tamanho
    ...
    def andar(self, direcao):
        ...
        if self.fim(AQUI):
            raise Erro, "Aaaaaaaaaaahhhhhhhh! (...) Ploft!"
        if self.mundo.colisao(self):
            raise Erro, "Olha por onde anda! Programa encerrado."

        pygame.time.wait(self.velocidade)


    def diga(self, * dizer):
        print " ".join([str(i) for i in dizer])
...
class Moedas(Objeto):
    def __init__(self, x, y, valor=None):
        ...
        font = pygame.font.Font("Courier New", 11)
        font.set_bold(True)
...
def diga(dizer):
    return Mundo.get_instance().principal.diga(dizer)
...
def qualquer(min, max):
    return random.randint(min, max)

def pode_ser(chance=1, em_cada=2):
    return qualquer(1,em_cada) <= chance
